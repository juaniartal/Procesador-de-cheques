...
"Por favor seleccione una opciones"
...
def Buscarcheque(filtro):
    busqueda = []
    cheques = readFile(urlfile)
    
    for cheque in cheques:
        if all(f(cheque) for f in filtro):
...
if __name__ == "__main__":
    while runtime:
        ...
        if op == "1":
            ...
            filtro = lambda cheque: cheque["DNI"] == dni, lambda cheque: estado.upper() in cheque["Estado"].upper(), lambda cheque: cheque['Tipo']== tipo
            print(urlfile, dni, tipo, salida)
            try:
                resultado = Buscarcheque(filtro)
                if salida == "PANTALLA":
                    print(resultado)
                elif salida == "CSV":
                    grabarCSV(resultado)
